...
class ASECGQuench(QuenchBase):
    ...
    def run(self, structure: Structure, *, step_callback=None) -> QuenchResult:
        try:
            ...
            raise ImportError("ASE is required for ASECGQuench") from exc
        ...
        if self._calculator is None:
            raise ValueError("ASECGQuench requires a calculator")
        atoms.set_calculator(self._calculator)

        opt = CG(atoms, logfile=None)
