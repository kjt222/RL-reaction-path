...
def _normalize_config_value(value: Any) -> Any:
    ...
    return _irrep_str(value)


def _apply_official_config(meta: dict[str, Any], model: nn.Module) -> None:
    """优先用 MACE 官方 extract_config 补齐/覆盖关键字段。"""
    try:
    ...
    if "atomic_numbers" in normalized:
        meta["z_table"] = [int(z) for z in normalized["atomic_numbers"]]
    if "atomic_energies" in normalized:
        meta["e0_values"] = normalized["atomic_energies"]
    if "num_bessel" in normalized:
        meta["num_radial_basis"] = int(normalized["num_bessel"])
    if "num_polynomial_cutoff" in normalized:
        meta["num_polynomial_cutoff"] = int(normalized["num_polynomial_cutoff"])
    ...
    if "atomic_inter_scale" in normalized or "atomic_inter_shift" in normalized:
        meta["scale_shift"] = {
            "scale": normalized.get("atomic_inter_scale", meta.get("scale_shift", {}).get("scale", 1.0)),
            "shift": normalized.get("atomic_inter_shift", meta.get("scale_shift", {}).get("shift", 0.0)),
        }
...
def _parse_max_ell_from_irreps(irreps_str: str) -> int | None:
    try:
        ...
        pass
    try:
        parts = re.findall(r"x(\d+)[eo]", irreps_str.replace(" ", ""))
        if parts:
...
def _infer_from_state_dict(sd: Mapping[str, Any]) -> dict:
    """Best-effort 推断核心架构/统计字段（命名遵循仓库根目录 parameters.md）。"""
...
def _export_model_json(model: nn.Module, json_path: Path) -> None:
    """提取/推断完整的模型 JSON，字段命名遵循仓库根目录 parameters.md。"""
    meta: dict[str, Any] = {}
    ...
    if hasattr(model, "atomic_energies_fn") and hasattr(model.atomic_energies_fn, "atomic_energies"):
        try:
            ...
            meta["e0_values"] = e0.detach().cpu().tolist()
        except Exception:
            meta["e0_values"] = _to_list(model.atomic_energies_fn.atomic_energies)
    if hasattr(model, "heads"):
        try:
            meta["heads"] = [str(h) for h in list(getattr(model, "heads"))]
        except Exception:
            meta["heads"] = _to_list(getattr(model, "heads"))
        ...
        if hasattr(model, "MLP_irreps_str"):
            ...
            if hasattr(model, "readouts"):
                for rd in model.readouts:
                    if hasattr(rd, "hidden_irreps"):
                        meta["MLP_irreps"] = _irrep_str(getattr(rd, "hidden_irreps"))
                        break
            if "MLP_irreps" not in meta:
    ...
    if "correlation" not in meta:
        for attr in ["correlation_meta", "correlation"]:
            if hasattr(model, attr):
                ...
                break
    if "correlation" not in meta and hasattr(model, "products"):
        try:
            for prod in model.products:
                contrs = getattr(prod, "symmetric_contractions", None)
                if contrs is None:
                    continue
                for contr in getattr(contrs, "contractions", []):
                    if hasattr(contr, "correlation"):
                        meta["correlation"] = int(torch.as_tensor(getattr(contr, "correlation")).item())
                        break
    ...
    if "gate" not in meta and hasattr(model, "interactions"):
        try:
            for blk in model.interactions:
                eq = getattr(blk, "equivariant_nonlin", None)
                if eq is None:
                    continue
                act = getattr(eq, "act_scalars", None)
                acts = getattr(act, "acts", None) if act is not None else None
                if acts:
                    fn = getattr(acts[0], "f", None)
                    if fn == torch.nn.functional.silu:
                        meta["gate"] = "silu"
                        break
                    if fn == torch.nn.functional.relu:
                        meta["gate"] = "relu"
                        break
                    if fn is not None:
                        meta["gate"] = getattr(fn, "__name__", str(fn))
                        break
        except Exception:
            pass
    ...
    if hasattr(model, "scale_shift"):
        ...
        try:
            scale = torch.as_tensor(ss.scale)
            shift = torch.as_tensor(ss.shift)
            meta["scale_shift"] = {
                "scale": scale.detach().cpu().view(-1).tolist() if scale.numel() > 1 else float(scale.item()),
                "shift": shift.detach().cpu().view(-1).tolist() if shift.numel() > 1 else float(shift.item()),
            }
        except Exception:
            pass
    ...
    # 官方 extract_config 可提供更完整的架构细节，优先覆盖关键字段
    _apply_official_config(meta, model)
    if hasattr(model, "use_edge_irreps_first"):
        meta["use_edge_irreps_first"] = bool(getattr(model, "use_edge_irreps_first"))
    if "apply_cutoff" not in meta and hasattr(model, "apply_cutoff"):
        meta["apply_cutoff"] = bool(getattr(model, "apply_cutoff"))
