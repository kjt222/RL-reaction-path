...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of frames to sample (default: 1).",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Radial cutoff (鑴? used to build neighborhoods (must match training).",
    )
    parser.add_argument(
        "--num_interactions",
        type=int,
...
def reservoir_sample_atoms(
    ...
    if total_seen < sample_size:
        logging.warning(
            "Requested %d samples but dataset only has %d frames; using all frames.",
            sample_size,
            total_seen,
        )
...
def main() -> None:
    ...
    write_predictions(sampled_atoms, energy_preds, force_preds, args.output)
